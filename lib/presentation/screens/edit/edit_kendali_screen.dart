...
class EditKendaliScreen extends StatefulWidget {
  final PaketPekerjaan package;
  final FotoKendali kendaliItem;

  const EditKendaliScreen({
    super.key,
    required this.package,
    required this.kendaliItem,
  });

  @override
  State<EditKendaliScreen> createState() => _EditKendaliScreenState();
}

class _EditKendaliScreenState extends State<EditKendaliScreen> {
  ...
  void initState() {
    super.initState();
    // Pre-fill data dari kendaliItem
    _progresFisik = widget.kendaliItem.majufreal > 0
        ? widget.kendaliItem.majufreal
        : widget.package.progresFisikSaatIni;
    _progresTextController.text = _progresFisik.toStringAsFixed(1);

    double initialKeuangan = widget.kendaliItem.majukeuangan > 0
        ? widget.kendaliItem.majukeuangan
        : (widget.package.realisasiKeuangan > 0
    ...
    _catatanController.text = widget.kendaliItem.keterangan;
    _info1Controller.text = widget.kendaliItem.info1;
    _info2Controller.text = widget.kendaliItem.info2;
  ...
  Future<void> _pickImage(int photoIndex, ImageSource source) async {
    try {
      ...
      try {
        ...
        burnSuccess = true;
        debugPrint('✅ Watermark edit ter-burn ke: $finalPath');
      } catch (burnError) {
        debugPrint('⚠️ Burn watermark edit gagal: $burnError');
        finalPath = pickedFile.path;
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Peringatan: Watermark gagal diterapkan. ($burnError)'),
  ...
  void _submitEditToApi() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        final success = await paketProvider.putEditKendali(
          kendaliId: widget.kendaliItem.kendaliId,
        ...
        if (mounted) {
          if (success) {
              ...
              builder: (ctx) => CupertinoAlertDialog(
                title: Row(
                  children: [
                    ...
                    Text('Berhasil Diperbarui', style: GoogleFonts.outfit()),
                  ],
                ),
                content: Padding(
                  padding: const EdgeInsets.only(top: 8.0),
                  child: Text(
                    'Perubahan lembar kendali #${widget.kendaliItem.kendaliId} berhasil disimpan.',
            ...
            _showErrorDialog(paketProvider.errorMessage ??
                'Gagal menyimpan perubahan lembar kendali.');
          }
        }
      } catch (e) {
      ...
      builder: (ctx) => CupertinoAlertDialog(
        title: Row(
          children: [
            ...
            const SizedBox(width: 8),
            Text('Gagal Memperbarui', style: GoogleFonts.outfit()),
          ],
        ),
        content: Padding(
          padding: const EdgeInsets.only(top: 8.0),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          'Edit Lembar Kendali #${widget.kendaliItem.kendaliId}',
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(
                              'Edit Kendali #${widget.kendaliItem.kendaliId}',
                              style: GoogleFonts.robotoMono(
                                fontSize: 10,
                                fontWeight: FontWeight.bold,
                                color: Colors.white,
                ...
                IosCard(
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(
                              _foto1Path != null ? 'Foto Baru' : 'Foto Saat Ini',
                              style: GoogleFonts.robotoMono(
                                fontSize: 10,
                                fontWeight: FontWeight.bold,
                                color: AppColors.primary,
                      ...
                      // Custom preview: jika file baru dipilih gunakan WatermarkPreview, jika belum tampilkan image dari URL existing dengan tombol ganti
                      if (_foto1Path != null)
                        WatermarkPreview(
                          ...
                          onRemove: () => setState(() => _foto1Path = null),
                        )
                      else
                        _buildExistingPhotoPreview(
                          url: widget.kendaliItem.foto1Url,
                          label: 'Foto 1 Saat Ini',
                          onPickCamera: () => _pickImage(1, ImageSource.camera),
                          onPickGallery: () => _pickImage(1, ImageSource.gallery),
                        ),
                      ...
                      IosTextField(
                        ...
                        hint: 'Contoh: Kondisi jembatan tampak depan...',
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(
                              _foto2Path != null ? 'Foto Baru' : 'Optional',
                              style: GoogleFonts.robotoMono(
                                fontSize: 10,
                                fontWeight: FontWeight.bold,
                      ...
                      if (_foto2Path != null)
                        WatermarkPreview(
                          ...
                          onRemove: () => setState(() => _foto2Path = null),
                        )
                      else
                        _buildExistingPhotoPreview(
                          url: widget.kendaliItem.foto2Url,
                          label: 'Foto 2 Saat Ini',
                          onPickCamera: () => _pickImage(2, ImageSource.camera),
                          onPickGallery: () => _pickImage(2, ImageSource.gallery),
                        ),
                      ...
                      IosTextField(
                        ...
                        hint: 'Contoh: Detail penulangan beton...',
                ...
                // Submit Button
                IosButton(
                  label: 'Simpan Perubahan Kendali',
                  icon: CupertinoIcons.checkmark_seal_fill,
                  isLoading: paketProvider.isLoading,
                  onPressed: _submitEditToApi,
                ),
                const SizedBox(height: 24),
              ],
            ),
  ...
  // ── Existing Photo Preview Widget ─────────────────────────────────────────

  Widget _buildExistingPhotoPreview({
    required String url,
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    ...
                    label: Text('Kamera', style: GoogleFonts.inter(fontSize: 12)),
                ...
                Expanded(
                  child: OutlinedButton.icon(
                    ...
                    label: Text('Galeri', style: GoogleFonts.inter(fontSize: 12)),
                    style: OutlinedButton.styleFrom(
          ...
          Expanded(
            child: Text(
              _isGettingLocation
                  ...
                  : (_hasLocation
                      ? 'GPS: ${_latitude!.toStringAsFixed(5)}, ${_longitude!.toStringAsFixed(5)} (±${_gpsAccuracy?.toStringAsFixed(0) ?? '?'}m)'
                      : 'GPS tidak aktif — Watermark tetap menyertakan waktu'),
              style: GoogleFonts.robotoMono(
                fontSize: 11,
                color: _hasLocation
                    ? AppColors.success
                ...
                fontWeight:
                    _hasLocation ? FontWeight.bold : FontWeight.normal,
              ),
            ),
          ),
        ],
      ...
      child: Row(
        ...
        children: [
          Icon(
            ...
            size: 13,
            color: isWatermarked ? AppColors.success : AppColors.warning,
          ),
          const SizedBox(width: 5),
          Text(
            isWatermarked
                ? 'Watermark GPS & Waktu Ter-burn ke File Foto'
                : 'Foto asli tanpa Watermark',
            style: GoogleFonts.inter(
              fontSize: 11,
              fontWeight: FontWeight.w600,
              color: isWatermarked ? AppColors.success : AppColors.warning,
